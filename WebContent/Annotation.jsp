...
<body>
	<fieldset>
		...
		<fieldset>
			...
			<br /> 
			<a href="<c:url value="/Annotation/isLogin.do"/>">로그인 여부 판단</a>
			\${param.error } : ${param.error }<br/>
			\${param.error2 } : ${param.error2 } <br/>
			<ul>
				<li>서블릿 API(HttpSession사용시) :${isLoginMessage }</li>
				<li>
					@SessionAttribute사용시 :${not empty param.error ? "비 로그인 상태":""}${isLoginMessage }
				</li>
			</ul>
			<h4>
